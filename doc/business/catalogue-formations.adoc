...
image::images/UE_details_onglet-Identif.pgn[]
...
 
image::images/UE_details_composPrincipe-1.pgn[]

image::images/UE_details_composPrincipe-2.pgn[]
...
o	Attributs présentés (tous en lecture seule)
	code
	filiation
	titulaires
	langue (calculée par subsidiarité à partir du container)
	statut
o	2 volumes horaires effectifs (calculé dynamiquement à partir des règles horaires des classes si elles existent) (+ codes couleur pour signaler la conformité de chaque classe avec les valeur nominales du composant)
...
 
image::images/UE_details_composClasses-1.pgn[]

image::images/UE_details_composClasses-2.pgn[]
...
Entité d’attribution	Container_year
(enregistré dans les détails de l’UE principale)	UE_year (partims)	
Anac de début	-	-	Calculé en recherchant le plus ancien record annuel de l’UE
Anac de fin	UE (petit chapeau)		
Site 	Container_year
(enregistré dans les détails de l’UE principale)	UE_year (partims)	
Périodicité 	UE (petit chapeau)		
Langue	Container_year
(enregistré dans les détails de l’UE principale)	Classe_year	

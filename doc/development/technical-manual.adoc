...
Developers should not commit directly to any of these branches. By convention, the source code can only be changed under the context of an issue created on the issue tracker tool. The issue tracker generates an incremental id that we can use to link a branch with the issue. In general, we name a branch after the issue’s id. Considering an issue with the id 260, we can create a local branch with the following commands:

    $ git fetch origin dev
    $ git checkout dev
    $ git checkout -b issue#260

The branch `dev` is updated with the last changes on the server and the branch `issue#260` is created from the branch `dev`. The option `-b` immediately switches to the new branch, to make sure that every commit from this point on is attached to the correct branch.
...
Committing often is encouraged. All commits are done locally, thus there is no risk of conflicts until all commits are sent to the server. The `push` option sends all commits in a local branch to the server, represented by `origin`.

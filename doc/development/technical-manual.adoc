...
The first thing to do to start contributing to the project is to prepare the development environment. During this section, we explain in details how to do that in a Debian-based distribution (Linux Debian, Ubuntu, Mint, etc.). The steps are:
...
Go to the new directory and clone the `dev` branch of OSIS modules:
    ...
    $ cd projects/osis
    $ git clone https://github.com/uclouvain/osis-core.git -b dev
    $ git clone https://github.com/uclouvain/osis-louvain.git -b dev
...
The repository `osis-louvain-frontend` is a Django application. To contribute to that, clone the repository locally:
    ...
    $ git clone https://github.com/uclouvain/osis-louvain-frontend.git -b dev
...
Don't do it now, but if you want to deactivate the virtual environment, simply type:

...
By convention, the source code can only be changed under the context of an issue created on the issue tracker tool. The issue tracker generates a unique id that we can use to link a branch with the issue. In general, we name a branch after the issue’s id. Considering an issue with the id 260, we can create a local branch with the following command:
